...
export function FloodingMiniGame({ isOpen, onOpenChange }: { isOpen: boolean; onOpenChange: (open: boolean) => void; }) {
  ...
  const handleChoice = (choice: 'safe' | 'unsafe') => {
    ...
    const isCorrect = (choice === 'safe' && currentItem.isSafe) || (choice === 'unsafe' && !currentItem.isSafe);
    ...
    if (isCorrect) {
      ...
      if (newScore === gameItems.length) {
        setGameWon(true);
        setShowConfetti(true);
        return;
      }

      const nextItem = items[newScore];
      setCurrentItem(nextItem);
    } else {
      setFeedback('wrong');
      const newLives = lives - 1;
      setLives(newLives);
      setWaterLevel(waterLevel + 33.33);
          ...
          <CardContent className="pt-8 pb-8 relative z-10">
            {!gameOver && !gameWon && currentItem && (
              <div className="space-y-6">
                ...
                <div className={`flex flex-col items-center justify-center p-8 bg-gradient-to-br from-blue-50 to-cyan-50 rounded-2xl border-4 border-blue-200 min-h-[280px] ${
                  feedback === 'correct' ? 'pulse-correct' : ''
                }`}>
                  <div className="w-32 h-32 rounded-full bg-white shadow-xl flex items-center justify-center mb-6 relative">
                    <IconComponent className="w-16 h-16 text-blue-600" />
                    <div className="absolute -top-2 -right-2 text-5xl">{currentItem.emoji}</div>
                  </div>
                  <p className="text-2xl font-bold text-gray-800 text-center max-w-lg">
                    {currentItem.text}
                  </p>
                </div>
                ...
                <div className="flex gap-4 justify-center">
                  <Button
                    ...
                    className="flex-1 max-w-xs bg-green-600 hover:bg-green-700 text-white text-xl py-8 font-bold shadow-lg hover:shadow-xl transition-all hover:scale-105"
                  >
                  ...
                  <Button
                    ...
                    className="flex-1 max-w-xs bg-red-600 hover:bg-red-700 text-white text-xl py-8 font-bold shadow-lg hover:shadow-xl transition-all hover:scale-105"
                  >
            ...
            {gameOver && (
              <div className="flex flex-col items-center justify-center py-12 space-y-6">
                ...
                <div className="text-center">
                  <h3 className="text-3xl font-bold text-gray-800 mb-2">Game Over!</h3>
                  <p className="text-xl text-gray-600 mb-4">You ran out of lives. Final Score: {score}/{gameItems.length}</p>
                  <p className="text-gray-500">Study flood safety tips and try again!</p>
                </div>
                <Button
                  onClick={startNewGame}
                  className="bg-blue-600 hover:bg-blue-700 text-lg py-6 px-12 font-semibold shadow-lg"
                >
                  Play Again
                </Button>
              </div>
            ...
            {gameWon && (
              <div className="flex flex-col items-center justify-center py-12 space-y-6">
                ...
                <div className="text-center">
                  <h3 className="text-3xl font-bold text-green-700 mb-2">Perfect Score! 🎉</h3>
                  <p className="text-xl text-gray-600 mb-4">You got all {gameItems.length} questions correct!</p>
                  <p className="text-gray-500">You're a flood safety expert!</p>
                </div>
                <Button
                  onClick={startNewGame}
                  className="bg-blue-600 hover:bg-blue-700 text-lg py-6 px-12 font-semibold shadow-lg"
