...
export function LectureCard({ lecture }: LectureCardProps) {
  ...
  const IconComponent = categoryIcons[lecture.category as keyof typeof categoryIcons] || BookOpen;
      ...
      >
        <div className={`h-48 bg-gradient-to-br ${gradient} relative overflow-hidden`}>
          <div className="absolute inset-0 flex items-center justify-center">
            <IconComponent className="text-6xl text-white opacity-20" />
          </div>
        ...
        <div className="p-5">
          ...
          <p className="text-sm text-muted-foreground mb-4 line-clamp-2" data-testid={`text-description-${lecture.id}`}>
            {lecture.content.replace(/<[^>]*>/g, '').substring(0, 120)}...
          </p>
